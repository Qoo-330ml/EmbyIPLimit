...
- 修复 webhook 通知链路与模板：支持测试、内置默认 title/content、NotifyHub body 自定义字段、游客求片/邀请注册/影子库同步/手动禁用启用通知
- 修复用户详情中到期禁用被误显示为多 IP 封禁的问题
- 修复手动禁用/启用通知中的用户名解析问题

### v1.4.4（2026-04-05）

- 修复求片偶发白屏bug；日志页性能优化；日志页可读性增强；后端日志体系增强；影子库异常日志增强；启动日志精简
