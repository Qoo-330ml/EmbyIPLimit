...
EmbyQ 是一个专门用于监控和限制 Emby 媒体服务器用户访问行为的工具。它能够实时监控用户的播放会话，检测异常登录行为（如同一用户在多个不同IP地址同时播放），并在达到阈值时自动禁用用户账号，提供完整的安全防护和访问控制功能。
...
> - 归属地查询依赖：`ip-hiofd==0.2.0`。
> - 相比旧版浏览器运行时镜像，体积显著缩减。
#### 2. 配置服务
首次启动后，程序会在 `/path/to/embyq/data` 目录下生成默认配置文件 `config.yaml`。
...
#### 1. 克隆项目
```bash
git clone https://github.com/Qoo-330ml/EmbyQ.git EmbyQ
cd EmbyQ
```
...
> 依赖说明（新环境）
> - Python 包：`requests`、`Flask`、`pyyaml`、`Werkzeug`、`flask_login`、`waitress`、`ip-hiofd==0.2.0`
> - 无需额外安装 Chromium / Playwright 浏览器运行时
> - CLI 命令：`ip-hiofd`（由对应 pip 包安装）
