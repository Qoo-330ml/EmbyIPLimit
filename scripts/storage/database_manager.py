...
class DatabaseManager(
    ...
    def init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            try:
                conn.execute('SELECT alert_threshold FROM user_expiry LIMIT 1')
            except sqlite3.OperationalError:
                conn.execute('ALTER TABLE user_expiry ADD COLUMN alert_threshold INTEGER')
                conn.commit()

            try:
                conn.execute('SELECT email FROM user_expiry LIMIT 1')
            except sqlite3.OperationalError:
                conn.execute('ALTER TABLE user_expiry ADD COLUMN email TEXT')
                conn.commit()
            ...
            try:
                conn.execute('SELECT alert_threshold FROM user_groups LIMIT 1')
            except sqlite3.OperationalError:
                conn.execute('ALTER TABLE user_groups ADD COLUMN alert_threshold INTEGER')
                conn.commit()
            ...
            try:
                conn.execute('SELECT target_email FROM invites LIMIT 1')
            except sqlite3.OperationalError:
                conn.execute('ALTER TABLE invites ADD COLUMN target_email TEXT')
                conn.commit()
            ...
            self._deduplicate_and_create_session_index(conn)

            conn.commit()

    def _deduplicate_and_create_session_index(self, conn):
        try:
            conn.execute('CREATE UNIQUE INDEX IF NOT EXISTS idx_session_id ON playback_history(session_id)')
            ...
            dup_session_ids = [row[0] for row in dup_cursor.fetchall()]
            for sid in dup_session_ids:
                conn.execute(
                    '''
