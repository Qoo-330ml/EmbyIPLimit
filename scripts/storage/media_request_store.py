...
class WishStore:
    ...
    def init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            self._ensure_column(conn, 'media_requests', 'user_id', "TEXT")
            ...
            conn.execute(
                ...
                CREATE TABLE IF NOT EXISTS media_request_comments (
                    ...
                    FOREIGN KEY (reply_to_id) REFERENCES media_request_comments(id) ON DELETE CASCADE
                )
                '''
            )
            self._ensure_column(conn, 'media_request_comments', 'reply_to_id', "INTEGER DEFAULT NULL")
            conn.execute(
                '''
                CREATE INDEX IF NOT EXISTS idx_comments_request_id
                ON media_request_comments (request_id, created_at ASC)
                '''
            )
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            cursor = conn.execute(
                ...
                INSERT INTO media_requests (
                    ...
                    overview, poster_path, poster_url, backdrop_path, backdrop_url, user_id,
                    status, updated_at
    ...
    def list_user_requests(self, user_id, page=1, page_size=25):
        try:
            page = max(int(page or 1), 1)
        except Exception:
            page = 1
    ...
    def list_all_requests_for_user(self, page=1, page_size=25):
        try:
            page = max(int(page or 1), 1)
        except Exception:
            page = 1
    ...
    def _rebuild_media_requests_table(self, conn):
        ...
        conn.execute(
            ...
            SELECT
                ...
                {"user_id" if has_user_id else "NULL"},
    ...
    def add_comment(self, request_id, user_id, username, content, reply_to_id=None):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            request_row = conn.execute(
                'SELECT id FROM media_requests WHERE id = ?',
                (request_id,),
            ).fetchone()
            if not request_row:
            ...
            if reply_to_id:
                reply_row = conn.execute(
                    'SELECT id, request_id FROM media_request_comments WHERE id = ?',
                    (reply_to_id,),
                ).fetchone()
                if not reply_row:
                    raise ValueError('回复的评论不存在')
                if dict(reply_row).get('request_id') != request_id:
                    raise ValueError('回复的评论不属于该求片记录')
            cursor = conn.execute(
                '''
                INSERT INTO media_request_comments (request_id, user_id, username, content, reply_to_id)
                VALUES (?, ?, ?, ?, ?)
                ''',
                (request_id, user_id, username, content, reply_to_id),
            )
            comment_id = int(cursor.lastrowid)
            conn.commit()
            row = conn.execute(
                '''
                SELECT id, request_id, user_id, username, content, reply_to_id, created_at
                FROM media_request_comments
                WHERE id = ?
                ''',
                (comment_id,),
            ).fetchone()
    ...
    def list_comments(self, request_id, page=1, page_size=50):
        try:
            page = max(int(page or 1), 1)
        except Exception:
            page = 1
        try:
            page_size = max(min(int(page_size or 50), 100), 1)
        except Exception:
            page_size = 50
        ...
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            total_row = conn.execute(
                'SELECT COUNT(1) FROM media_request_comments WHERE request_id = ?',
                (request_id,),
            ).fetchone()
            total_results = int(total_row[0] or 0) if total_row else 0
            rows = conn.execute(
    ...
    def delete_comment(self, comment_id, user_id=None):
        with sqlite3.connect(self.db_path) as conn:
            if user_id:
                cursor = conn.execute(
                    'DELETE FROM media_request_comments WHERE id = ? AND user_id = ?',
                    (comment_id, user_id),
                )
            else:
                cursor = conn.execute(
                    'DELETE FROM media_request_comments WHERE id = ?',
                    (comment_id,),
                )
            conn.commit()
            return cursor.rowcount > 0

    def get_wish_user_info(self, request_id):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    'SELECT title, media_type, user_id FROM media_requests WHERE id = ?',
                    (request_id,),
                )
                row = cursor.fetchone()
                if not row:
                    return None
                return {'title': row[0], 'media_type': row[1], 'user_id': row[2]}
        except Exception:
            return None
    ...
    def get_comment_user_info(self, comment_id):
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.execute(
                    'SELECT user_id, username FROM media_request_comments WHERE id = ?',
                    (comment_id,),
                )
                row = cursor.fetchone()
                if not row:
