...
from web.helpers import admin_required, send_webhook_if_enabled, to_int
...
def register_admin_shadow_routes(server):
    ...
    def admin_shadow_stats():
        ...
        try:
            ...
            logger.exception('获取影子库统计失败: %s', exc)
            return jsonify({'error': '获取影子库统计失败'}), 500
    ...
    def admin_shadow_movies():
        ...
        try:
            ...
            logger.exception('获取影子库电影列表失败: %s', exc)
            return jsonify({'error': '获取影子库电影列表失败'}), 500
    ...
    def admin_shadow_series():
        ...
        try:
            ...
            logger.exception('获取影子库剧集列表失败: %s', exc)
            return jsonify({'error': '获取影子库剧集列表失败'}), 500
    ...
    def admin_shadow_series_detail(emby_id):
        ...
        try:
            ...
            logger.exception('获取剧集详情失败: %s', exc)
            return jsonify({'error': '获取剧集详情失败'}), 500
    ...
    def admin_shadow_search():
        ...
        try:
            ...
            logger.exception('影子库搜索失败: %s', exc)
            return jsonify({'error': '影子库搜索失败'}), 500
