...
def register_public_routes(server):
    ...
    def public_search():
        ...
        playback_records = server.serialize_playback_records(
            server.get_user_playback_records(user_id=user_id, username=username)
        )
        ban_info = server.serialize_ban_info(server.get_user_ban_info(user_id=user_id, username=username))
