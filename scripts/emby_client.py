...
class EmbyClient:
    ...
    def update_config(self, server_url, api_key):
        self.server_url = server_url.rstrip('/')
        self.api_key = api_key
        self.session = create_session({'X-Emby-Token': self.api_key})
        logger.info('🔄 EmbyClient 配置已更新: server_url=%s', self.server_url)
    ...
    def authenticate_user(self, username, password):
        try:
            headers = {
                'X-Emby-Authorization': 'Emby Client="EmbyQ", Device="EmbyQ Web", DeviceId="embyq-web", Version="1.0.0"',
                'X-Emby-Token': self.api_key,
                'Content-Type': 'application/x-www-form-urlencoded',
            }
            response = self.session.post(
                f"{self.server_url}/emby/Users/AuthenticateByName",
                headers=headers,
                data=f'Username={username}&Pw={password}',
                timeout=8,
            )
            if response.status_code == 200:
                ...
                return user_info
            logger.warning('Emby用户认证失败: username=%s, status_code=%s, response=%s', username, response.status_code, response.text[:200])
            return None
        except Exception as e:
            logger.warning('Emby用户认证异常: username=%s, error=%s', username, e)
            return None
    ...
    def update_user_policy_fields(self, user_id, updates):
        try:
            ...
            if not current_policy:
                logger.warning('获取用户策略为空，尝试使用默认策略: user_id=%s', user_id)
                current_policy = {}
    ...
    def get_backdrop_posters(self, limit=5):
        try:
            fields = 'BackdropImageTags,Name'
            all_movies = self.get_movies(fields=fields)
            all_series = self.get_series_list(fields=fields)
            all_items = all_movies + all_series
            items_with_backdrop = [
                item for item in all_items
                if item.get('BackdropImageTags') and len(item['BackdropImageTags']) > 0
            ]
            import random
            selected = random.sample(items_with_backdrop, min(limit, len(items_with_backdrop))) if items_with_backdrop else []
            results = []
            for item in selected:
                results.append({
                    'url': f"{self.server_url}/emby/Items/{item['Id']}/Images/Backdrop/0",
                    'title': item.get('Name', '未知内容'),
                })
            return results
        except Exception as e:
            logger.warning('获取背景海报失败: error=%s', e)
            return []
