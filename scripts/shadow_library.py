...
class ShadowLibrary:
    ...
    def get_movies(self, page=1, page_size=20):
        return self._get_paginated_items('Movie', page, page_size)

    def get_series_list(self, page=1, page_size=20):
        return self._get_paginated_items('Series', page, page_size)

    def _get_paginated_items(self, media_type, page=1, page_size=20):
        offset = (page - 1) * page_size
        with sqlite3.connect(self.db_path) as conn:
            conn.row_factory = sqlite3.Row
            total = conn.execute(
                "SELECT COUNT(*) FROM shadow_library WHERE media_type = ?",
                (media_type,)
            ).fetchone()[0]
            rows = conn.execute('''
                ...
                ORDER BY name
                LIMIT ? OFFSET ?
            ''', (media_type, page_size, offset)).fetchall()
            return {
